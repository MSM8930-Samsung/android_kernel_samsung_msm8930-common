...
//#include <linux/battery/sec_charger.h>	//montest
#define CONFIG_BATTERY_SAMSUNG_EXCLUDE	0
...
#if 1// !defined(CONFIG_BATTERY_SAMSUNG_EXCLUDE)
...
static unsigned int voltage_based_capacity(struct pm8921_chg_chip *chip)
{
	...
	static unsigned int capa_counter;
	...
	if (current_voltage_mv <= low_voltage)
		...
		if (capa_counter < 10) {
			chip->capacity_value[capa_counter]
				= (current_voltage_mv - low_voltage) * 100
				/ (high_voltage - low_voltage);
			capacity_sum += chip->capacity_value
				[capa_counter];
			capa_counter++;
			return capacity_sum / capa_counter;
		} else {
			capacity_new = (current_voltage_mv - low_voltage) * 100
				/ (high_voltage - low_voltage);
			...
			capacity_new = (capacity_sum - capa_max - capa_min
					+ capacity_new) / 9;
			capacity_sum -= chip->capacity_value
				[capa_counter - 10];
			chip->capacity_value[capa_counter - 10] = capacity_new;
			capacity_sum += chip->capacity_value[capa_counter - 10];
			capa_counter++;
			...
			struct pm8921_chg_chip *chip, int raw_soc)
{
	...
	chip->capacity_max =
		((chip->capacity_max - chip->batt_pdata->capacity_min)
		* 99 / 100) + chip->batt_pdata->capacity_min;
...
static int get_prop_batt_status(struct pm8921_chg_chip *chip)
{
	...
	int batt_capacity = get_prop_batt_capacity(chip);
	...
	if (chip->cable_type == CABLE_TYPE_INCOMPATIBLE || ovp_state) {
		chip->batt_status = POWER_SUPPLY_STATUS_NOT_CHARGING;
...
#ifndef CONFIG_CHARGER_MAX77693
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_AVG:
		val->intval = get_prop_battery_uvolts(chip);
#if defined(CONFIG_BATTERY_SAMSUNG_EXCLUDE)
		val->intval /= 1000;
#endif
		break;
	case POWER_SUPPLY_PROP_CAPACITY:
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (new_cable_type == chip->cable_type
				&& !ovp_state)
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		...
		pm8921_charger_vbus_draw(val->intval);
		pm8921_set_max_battery_charge_current(val->intval);
		break;
#endif
	default:
		return -EINVAL;
...
static void handle_cable_insertion_removal(struct pm8921_chg_chip *chip)
{
	...
	wake_lock_timeout(&chip->monitor_wake_lock, 5*HZ);
...
#ifdef CONFIG_WIRELESS_CHARGING
	if(chip->cable_type == CABLE_TYPE_WPC)
		msleep(300);
#endif
