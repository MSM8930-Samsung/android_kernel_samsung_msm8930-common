	...
	if (system_rev >= BOARD_REV03) {
		if (vsensor_3p0 == NULL) {
			...
			ret = regulator_set_voltage(vsensor_3p0,
					3000000, 3000000);
			if (ret)
				pr_err("%s: error vsensor_3p0 setting voltage ret=%d\n",
						__func__, ret);
		}
		if (onoff == SNS_PWR_ON) {
			ret = regulator_enable(vsensor_3p0);
			if (ret)
				pr_err("%s: error enablinig vsensor_3p0 regulator\n",
						__func__);
...
void __init msm8930_express2_init(void)
{
#ifdef CONFIG_SEC_DEBUG
	sec_debug_init();
#endif
	...
	msm_otg_power_init(GPIO_OTG_TEST, 1);
#endif
}

MACHINE_START(EXPRESS2, "SAMSUNG EXPRESS2")
	.map_io = msm8930_map_io,
	.reserve = msm8930_reserve,
	.init_irq = msm8930_init_irq,
	.handle_irq = gic_handle_irq,
	.timer = &msm_timer,
	.init_machine = msm8930_express2_init,
